...
def words(entry):
    ...
    return [word.lower().decode('utf-8', 'ignore') for word in entry.split()]
...
def letters(entry):
    ...
    return [letter for letter in list(entry.lower().decode('utf-8', 'ignore')) if letter not in string.punctuation]
...
def ply_markov(entry, ply, current_dict, split_callback):
    ...
    for i in range(0, len(words)-ply):
        current_tuple = tuple([words[j] for j in range(i, i+ply)])
...
def train(source, ply, split_callback=words, master_dict=None):
    ...
    master_dict = master_dict or {i: {} for i in range(1,ply+1)}
    for line in source:
        #print(line)
...
def get_check_tuple(current_output, ply):
    ...
    last_n_list = [current_output[-i] for i in range(1,ply+1)]
    last_n_list.reverse()
...
def append_next_word(master_dict, current_output, ply):
    ...
    for i in range(1, ply+1):
        ...
        ply_list.extend([[key]*value*i for key, value in check.items()])
